...
| SRS v2 - Root Certificate-Paket    | Softwarepaket       | Paket aus, das Stammzertifikat (nicht erforderlich für die Domäne eingebundener Einheiten) bereitgestellt.          |
| SRS v2 - OMS Microsoft Agent-Paket | Softwarepaket       | Paket zum Bereitstellen und Konfigurieren des Microsoft Operations Management Suite-Agents        |
...
1.  [**Herunterladen**]() der Aufgabensequenz Sample, und kopieren Sie die heruntergeladene Zip-Datei an einen freigegebenen Speicherort.
...
1.  Durch die einzelnen Schritte durchlaufen Sie, und schließen Sie die empfohlenen Updates:
    ...
    9.  **Stammzertifikat installieren**: in diesem Schritt das Stammzertifikat für Geräte nicht in die Domäne eingebundener verteilt und daher ist optional.
        -   Entfernen Sie oder deaktivieren Sie diesen Schritt, wenn Sie ein Stammzertifikat für die Skype Raum Systemen v2 Einheiten bereitstellen müssen nicht.
        -   Wenn Sie benötigen, um diesen Schritt ausführen, stellen Sie sicher, dass der **SRS v2 – Root Certificate Paket** aktiviert ist.
    ...
    10. **Installieren und Konfigurieren von OMS-Agent**: in diesem Schritt installiert die 64-Bit-Version des Microsoft Operations Management Suite-Agents und den Verbindung mit den Arbeitsbereich Protokoll Analytics-Agent konfiguriert.
        -   Deaktivieren Sie diesen Schritt nur aus, wenn Sie einige andere Plattformen verwenden also, um die Integrität der Einheiten v2 Skype Raum Systeme überwachen.
        ...
        -   Stellen Sie sicher, dass der **SRS v2 – Microsoft OMS-Agent-Paket** aktiviert ist.
    ...
    14. **Skype-Benutzer**: Dieser Schritt wird das lokale Skype-Konto verwendet, um automatisch bei Windows anmelden und starten Sie die Skype Raum Systemen v2-Anwendung erstellt. Dieser Schritt keine Softwarepaket zugeordnet haben, und keine Anpassung für die es erforderlich ist.

    15. **Festlegen einrichten und Konfigurieren der SRS Anwendung**: in diesem Schritt installiert und konfiguriert die Skype Raum Systemen v2-Anwendung. Dieser Schritt die lokal kopierten Bit verwendet, um die Anwendung installieren und daher nicht haben alle Softwarepakete zugeordnet ist. Keine Anpassung ist für diesen Schritt erforderlich.
...
> [!WARNING]
> Es ist sehr wichtig, dass **Zweck** auf **verfügbar**festgelegt ist. Stellen Sie sicher, dass der **Zweck** **nicht** auf **erforderlich**festgelegt ist. Stellen Sie außerdem sicher, dass Sie in **der folgenden zur Verfügung stellen** **nur Medien und PXE** auswählen. Festlegen von diese Werte auf einen anderen Suchbegriff möglicherweise alle Computer mit dem Skype Raum Systeme Bereitstellungsabbild Wenn gestartet wird.
...
> [!TIP]
> Sie können F8 auswählen, können Sie jederzeit während der Aufgabensequenz, öffnen eine Befehlskonsole und erhalten Sie Zugriff auf die Masterprotokolldatei.

Um PXE Boot Probleme zu beheben, überprüfen Sie die zwei Protokolldateien auf dem Server-Konfigurations-Manager, die speziell für PXE-Aktionen sind:
