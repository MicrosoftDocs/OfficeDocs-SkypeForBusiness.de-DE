...
SQL bewährte Methoden in Bezug darauf, welche SQL Versionen für eine Zeugenrolle unterstützt werden, finden Sie unter ["Datenbankspiegelungszeugen](/sql/database-engine/database-mirroring/database-mirroring-witness)".
...
> [!NOTE]
> Eine Dateifreigabe muss erstellt werden, damit die Spiegeldateien geschrieben werden, und der Dienst, in dem SQL Server und SQL Agent ausgeführt wird, benötigt Lese-/Schreibzugriff. Wenn der SQL Server Dienst im Kontext des Netzwerkdiensts ausgeführt wird, können Sie den Freigabeberechtigungen<SQLSERVERNAME\>$ des Prinzipalservers und des Spiegelservers SQL hinzufügen.\<Domain\>\\ $ist wichtig, um zu erkennen, dass es sich um ein Computerkonto handelt.
...
1. Klicken **Sie auf der Seite SQL Store** definieren neben dem **SQL Store-Feld** auf **Neu**.
...
3. Veröffentlichen Sie die Topologie.
   ...
   ```
    ...
    Folgen Sie diesem Schritt jedoch nicht, und geben  `Uninstall-CsMirrorDatabase` Sie nicht ein, dass dadurch die gesamte Spiegelungskonfiguration deinstalliert würde.
