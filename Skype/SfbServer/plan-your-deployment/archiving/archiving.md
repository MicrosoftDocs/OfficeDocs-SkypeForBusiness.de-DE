...
- Auswahl der Integrationsoption "Microsoft Exchange" zur Verwendung von Exchange für die Speicherung archivierter Daten
...
Wenn Sie Skype for Business Server Speicher auswählen, verwenden Sie Skype for Business Server Archivierungsrichtlinien und -konfigurationen, um zu steuern, wie die Archivierung aktiviert und implementiert wird. Skype for Business Server Speicher SQL Server Datenbanken verwendet. Daher müssen Sie die entsprechenden SQL Server Datenbanken zu Ihrer Topologie hinzufügen und dann Ihre Archivierungsrichtlinien konfigurieren. 
...
- **Microsoft Exchange-Speicher**. Wenn Sie die Exchange Integrationsoption auswählen, wird der Archivierungsinhalt Skype for Business Server im Exchange Speicher für alle Benutzer abgelegt, die in Exchange verwaltet werden und deren Postfächer in In-Place Haltebereich gesetzt wurden. Archivierte Daten werden in den Benutzerpostfächern des Ordners "Wiederherstellbare Elemente" gespeichert, der für Benutzer im Allgemeinen nicht sichtbar ist und nur von Benutzern mit einer Exchange **Discovery-Verwaltungsrolle** durchsucht werden kann. Exchange ermöglicht die Sammelsuche und -ermittlung sowie SharePoint, wenn sie bereitgestellt wird. Weitere Informationen zur Speicherung, Aufbewahrung und Ermittlung von in Exchange gespeicherten Daten finden Sie in der Dokumentation zu Exchange und SharePoint.
    
- **Skype for Business Server Archivierungsspeicher.** Wenn Sie Skype for Business Server Archivierungsdatenbanken einrichten, Skype for Business Server Archivierungsinhalte in den Skype for Business Server Archivierungsdatenbanken für alle Benutzer, die nicht in Exchange verwaltet werden, und die ihre Postfächer nicht aktiviert haben, abspeichern. In-Place Halten. Diese Daten sind nicht durchsuchbar, können aber in Formate exportiert werden, die mit anderen Tools durchsuchbar sind. Ausführliche Informationen zum Exportieren von in Archivierungsdatenbanken gespeicherten Daten finden Sie unter [Exportieren archivierter Daten in Skype for Business Server](../../manage/archiving/export-archived-data.md).
