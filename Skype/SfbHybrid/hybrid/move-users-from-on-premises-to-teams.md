...
- Der Benutzer kann mit anderen Benutzern zusammenarbeiten, die Skype for Business verwenden (entweder online oder lokal).
...
- Benutzer können weiterhin an Skype for Business Besprechungen teilnehmen.
...
- Lokale Kontakte sind in Teams verfügbar, kurz nachdem sich der Benutzer zum ersten Mal angemeldet hat.
...
> [!IMPORTANT]
>Wenn ein Benutzer mit Move-CsUser von der lokalen Umgebung in die Cloud verschoben wird, wird benutzern jetzt automatisch der TeamsOnly-Modus zugewiesen, und ihre Besprechungen aus der lokalen Umgebung werden automatisch in Teams Besprechungen konvertiert, unabhängig davon, ob der `-MoveToTeams` Switch tatsächlich angegeben ist. (Dies umfasst Migrationen von Lync Server 2013, die nie den `-MoveToTeams` Switch hatten.)  Wenn dieser Switch nicht angegeben wurde, haben Benutzer zuvor von der lokalen Skype for Business Server zu Skype for Business Online gewechselt, und ihr Modus bleibt unverändert. Dies wurde kürzlich in Vorbereitung auf die Einstellung von Skype for Business Online geändert.
...
Move-CsUser ist in einem lokalen PowerShell-Fenster Skype for Business Server Verwaltungsshell oder in einem PowerShell-Fenster der Lync Server-Verwaltungsshell verfügbar. So verschieben Sie einen Benutzer mit Move-CsUser in den TeamsOnly-Modus:
