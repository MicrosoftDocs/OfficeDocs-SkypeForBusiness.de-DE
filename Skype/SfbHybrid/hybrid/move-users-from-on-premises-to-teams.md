...
- Der Benutzer kann weiterhin an Skype for Business Besprechungen teilnehmen.
...
Überprüfen Sie vor dem Verschieben von Benutzern unbedingt die [Voraussetzungen,](move-users-between-on-premises-and-cloud.md#prerequisites) um Benutzer in die Cloud zu verschieben. Überprüfen Sie außerdem die [Migrations- und Interoperabilitätsleitlinien für Organisationen, die Teams zusammen mit Skype for Business verwenden.](/microsoftteams/migration-interop-guidance-for-teams-with-skype)


> [!NOTE]
> Einheitliche Kontakt-Store sollten für das lokale SfB-Konto deaktiviert werden, damit der Kontakt zu Teams verschoben werden kann.

> [!IMPORTANT]
>
> - Wenn ein Benutzer mit Move-CsUser von der lokalen Bereitstellung in die Cloud verschoben wird, wird benutzern jetzt automatisch der TeamsOnly-Modus zugewiesen, und ihre Besprechungen aus der lokalen Umgebung werden automatisch in Teams Besprechungen konvertiert, unabhängig davon, ob der `-MoveToTeams` Switch tatsächlich angegeben ist. (Dies umfasst Migrationen von Lync Server 2013, die nie den `-MoveToTeams` Switch hatten.)  Wenn dieser Switch nicht angegeben wurde, wurden Benutzer zuvor von Skype for Business Server lokal zu Skype for Business Online verwaltet, und ihr Modus bleibt unverändert. Dies wurde kürzlich in Vorbereitung auf die Einstellung von Skype for Business Online geändert.
> - Das Verschieben von Benutzern zwischen Ihrer lokalen Bereitstellung und Teams *erfordert* jetzt das OAuth-Authentifizierungsprotokoll. Zuvor wurde OAuth empfohlen, aber nicht erforderlich.  Skype for Business Server 2019 und Skype for Business Server 2015 CU12 (KB 3061064) erfordert bereits OAuth. Wenn Sie Skype for Business Server 2015 mit CU8 bis CU11 verwenden, müssen Sie den `-UseOAuth` Switch übergeben, der sicherstellt, dass der lokale Code mit OAuth authentifiziert wird oder vorzugsweise auf CU12 aktualisiert wird. Wenn Sie eine Version von Skype for Business Server 2015 vor CU8 verwenden, müssen Sie ein Upgrade auf CU12 oder höher durchführen.  Wenn Sie Lync Server 2013 verwenden, müssen Sie zuerst ein Upgrade auf lync Server 2013 Kumulatives Update 10 Hotfix 5 (KB 2809243) oder höher durchführen.
...
Die folgende Cmdletsequenz kann verwendet werden, um einen Benutzer nach TeamsOnly zu verschieben. Dabei wird davon ausgegangen, dass die Microsoft 365 Anmeldeinformationen ein separates Konto ist und als Eingabe für die Get-Credential Eingabeaufforderung angegeben wird. Das Verhalten ist unabhängig davon, ob `-MoveToTeams` ein Schalter angegeben ist, identisch.
...
4. Wählen Sie die Benutzer aus, und wählen Sie dann im Dropdownmenü **"Aktion"** über der Liste die Option **"Ausgewählte Benutzer in Teams** verschieben" oder **"Ausgewählte Benutzer in Skype for Business Online verschieben"** aus.   Beide Optionen verschieben Benutzer jetzt direkt zu TeamsOnly.
