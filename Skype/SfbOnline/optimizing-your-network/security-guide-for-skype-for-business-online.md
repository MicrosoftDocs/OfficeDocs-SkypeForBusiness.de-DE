...
Skype for Business Online verwendet die PKI-Features im Betriebssystem Windows Server, um die Schlüsseldaten zu schützen, die für die Verschlüsselung der TLS-Verbindungen (Transport Layer Security) verwendet werden. Die für die Medienverschlüsselungen verwendeten Schlüssel werden über TLS-Verbindungen ausgetauscht. 
...
SfBO mildert diese Angriffe, indem Azure DDOS-Netzwerkschutz ausgeführt und Clientanforderungen von denselben Endpunkten, Subnetzen und Verbundentitäten drosselt werden.
...
SfBO verwendet mutual TLS (MTLS) für die Serverkommunikation innerhalb Microsoft 365 oder Office 365 und TLS von Clients zum Dienst, wodurch dieser Angriff innerhalb des Zeitraums, in dem eine bestimmte Unterhaltung angegriffen werden könnte, sehr schwierig bis unmöglich zu erreichen ist. TLS authentifiziert alle Parteien und verschlüsselt den gesamten Datenverkehr. Damit können Abhöraktionen nicht verhindert werden, aber Angreifer können den Datenverkehr nicht lesen, es sei denn, die Verschlüsselung geht verloren.
...
Dies ist für TLS-Verbindungen (Transport Layer Security) weniger ein Problem, da TLS alle Parteien authentifiziert und den gesamten Datenverkehr verschlüsselt. Die Verwendung von TLS verhindert Spoofingangriffe auf bestimmte Verbindungen (Mutual TLS-Verbindungen). Aber ein Angreifer könnte immer noch die Adresse des DNS-Servers spoofieren, den SfBO verwendet. Da die Authentifizierung in SfBO jedoch mit Zertifikaten durchgeführt wird, verfügt ein Angreifer nicht über ein gültiges Zertifikat, das erforderlich ist, um eine der Parteien in der Kommunikation zu spoofieren.
...
| Kalenderdaten | Frei/Gebucht, Abwesenheitshinweis, Besprechungsdetails (für diejenigen, die Zugriff auf Ihren Kalender haben) |
...
Ein vertrauenswürdiger Benutzer ist einer, dessen Anmeldeinformationen von AAD in Microsoft 365 oder Office 365 authentifiziert wurden. 
...
Für die Medienauthentifizierung verwenden die ICE- und TURN-Protokolle auch die Digest-Challenge, wie im IETF TURN RFC beschrieben. Ausführliche Informationen finden Sie [unter Medien traversal](#external-user-av-traffic-traversal).

Clientzertifikate bieten eine alternative Möglichkeit für Benutzer, von SfBO authentifiziert zu werden. Anstelle der Angabe eines Benutzernamens und eines Kennworts haben die Benutzer ein Zertifikat und den privaten Schlüssel, der dem Zertifikat entspricht, das zum Auflösen einer kryptografischen Aufforderung benötigt wird. 
...
Die Client verwenden die Ports UDP 3478-3481 und TCP 443, um Dienste vom A/V Edge-Dienst anzufordern. Ein Client verwendet diese beiden Ports, um UDP- bzw. TCP-Ports für die Verbindung zum Remote-Teilnehmer zuzuweisen. Um auf den A/V-Edgedienst zugreifen zu können, muss der Client zuerst eine authentifizierte SIP-Signalsitzung mit der SfBO-Registrierungsstelle eingerichtet haben, um A/V-Edgedienstauthentifizierungsanmeldeinformationen abzurufen. Diese Werte werden über den TLS-geschützten Signalisierungskanal gesendet und sind computergeneriert, um gegen Schlüsselverzeichnisangriffe zu schützen. Clients können diese Anmeldeinformationen dann für die Hashwert-Authentifizierung mit dem A/V Edge-Dienst verwenden, um Ports für die Verwendung in einer Mediensitzung zuzuweisen. Eine erste Zuordnungsanforderung wird vom Client gesendet und mit einer 401 Nonce/Aufforderung-Nachricht vom A/V Edge-Dienst beantwortet. Der Client sendet eine zweite Zuweisung, die den Benutzernamen und einen Hash Message Authentication Code (HMAC) Hash des Benutzernamens und nonce enthält. 
...
![Anrufsequenz in der Besprechungsteil nehmen an.](media/sfbo-call-sequence-security.png) 
...
- **Veranstalter** &nbsp;&nbsp; Der Benutzer, der eine Besprechung erstellt, sei es spontan oder durch Planung. Ein Organisator muss ein authentifizierter Unternehmensbenutzer sein und die Kontrolle über alle Endbenutzeraspekte einer Besprechung haben.
...
- **Teilnehmer** &nbsp;&nbsp; Ein Benutzer, der zur Teilnahme an einer Besprechung eingeladen wurde, aber nicht berechtigt ist, als Referent zu fungieren.
...
- **Jeder aus meiner Organisation**&nbsp;&nbsp; Alle SfBO-Benutzer im Microsoft 365 oder Office 365 Mandanten können an der Besprechung teilnehmen, ohne im Wartebereich zu warten, auch wenn diejenigen, die sich nicht in der Verteilerliste befinden. Alle anderen, einschließlich aller externen und anonymen Benutzer, müssen in der Lobby warten, bis sie zugelassen werden.
- **Jemand**&nbsp;&nbsp; Jeder (es gibt keine Einschränkungen), der Zugriff auf den Besprechungslink hat, kommt direkt in die Besprechung.
...
- **Alle, auch Personen außerhalb meines Unternehmens**&nbsp;&nbsp; Jeder (es gibt keine Einschränkungen), der an der Besprechung teilnimmt, kann präsentieren.
