---
title: Wie sich unterschiedliche Technologien auf die Anmeldung bei Microsoft Teams auswirken, einschließlich der Einschränkung von Anmelde- und Anmeldeverhalten.
author: MSFTTracyP
ms.author: tracyp
manager: dansimp
audience: Admin
...
ms.reviewer: anwara
description: Erfahren Sie, wie Technologien wie einmaliges Anmelden und moderne Authentifizierung Ihr Anmeldeverhalten unter iOS, Android, macOS und PC beeinflussen. Verwenden von Teams mit vielen Konten und Einschränken der Anmeldung Es wird auch erläutert, wie man Microsoft Teams anweisen kann, das Vorausfüllen des Benutzernamens (UPN) bei der Anmeldung zu ignorieren.
...
---
# <a name="how-different-technologies-effect-microsoft-teams-sign-on"></a>Auswirkungen verschiedener Technologien auf die Anmeldung bei Microsoft Teams

Wenn Sie wissen müssen, wie Sich Technologien wie einmaliges Anmelden (Single Sign-On, SSO), moderne Authentifizierung (MS) und mehrstufige Authentifizierung (MA) auf die Anmeldeerfahrung der Benutzer auswirken, wird in diesem Artikel erläutert, was Benutzer und Administratoren erwarten können. Außerdem wird das Anmeldeverhalten für macOS-, Android- und iOS-Geräte, die Funktionsweise der Anmeldung mit mehreren Konten, das Entfernen automatisch ausgefüllter Anmeldeinformationen oder die "Vorabausfüllung" auf dem Anmeldebildschirm und das Einschränken der Anmeldung erläutert.

Setzen Sie ein Lesezeichen für diesen Artikel, wenn Ihre Rolle darin besteht, das erwartete Verhalten von Microsoft Team während der Anmeldung zu kennen.

## <a name="microsoft-teams-and-windows-users-sign-in-recommendations"></a>Microsoft Teams- und Windows-Benutzer: Anmeldeempfehlungen
...
### <a name="how-modern-authentication-ma-effects-your-sign-in-what-users-will-see-when-ma-is-on"></a>Auswirkungen der modernen Authentifizierung (MA) auf Ihre Anmeldung: Was Benutzern angezeigt wird, wenn MA aktiviert ist

Die moderne Authentifizierung ist Teil eines Prozesses, der Teams darüber informiert, dass Benutzer ihre Anmeldeinformationen ( z. B. ihre geschäftliche E-Mail und ihr Kennwort ) bereits an anderer Stelle eingegeben haben, und sie sollten nicht erneut eingeben müssen, um die App zu starten. Die Benutzererfahrung hängt von einigen Faktoren ab, z. B. ob Benutzer in einem Windows-Betriebssystem oder auf einem Mac arbeiten.

Das Anmeldeverhalten variiert auch je nachdem, ob Ihre Organisation die einstufige Authentifizierung oder die mehrstufige Authentifizierung aktiviert hat. Die mehrstufige Authentifizierung umfasst in der Regel die Überprüfung von Anmeldeinformationen über ein Telefon, die Bereitstellung eines eindeutigen Codes, das Eingeben einer PIN oder die Verwendung eines Fingerabdrucks. 
...
Nachfolgend finden Sie eine Übersicht über das Verhalten, das Benutzer bei jedem modernen Authentifizierungsszenario erwarten können.
...
- Wenn SSO nicht möglich ist, füllt Teams auf PCs, die einer Domäne beigetreten sind, den Anmeldebildschirm möglicherweise vorab mit dem Benutzerprinzipalnamen (USER Principal Name, UPN) aus. Es gibt Fälle, in denen Sie dies möglicherweise nicht möchten, insbesondere wenn Ihre Organisation verschiedene UPNs lokal und in Azure Active Directory verwendet. In diesem Fall **können Sie den folgenden Windows-Registrierungsschlüssel verwenden, um die Vorgesamtheit des UPN zu deaktivieren**:
...
### <a name="microsoft-teams-sign-on-to-another-account-on-a-domain-joined-computer"></a>Anmelden von Microsoft Teams bei einem anderen Konto auf einem Computer, der einer Domäne beigetreten ist
...
## <a name="macos-users-and-microsoft-teams-sign-on-prompts"></a>MacOS-Benutzer und Microsoft Teams-Anmeldeaufforderungen
...
## <a name="microsoft-teams-sign-on-for-ios-and-android-users"></a>Microsoft Teams-Anmeldung für iOS- und Android-Benutzer
...
## <a name="using-microsoft-teams-with-multiple-sign-in-accounts"></a>Verwenden von Microsoft Teams mit mehreren Anmeldekonten
...
## <a name="restrict-sign-in-to-microsoft-teams"></a>Einschränken der Anmeldung bei Microsoft Teams
...
### <a name="how-to-restrict-microsoft-teams-sign-in-on-mobile-devices"></a>Einschränken der Microsoft Teams-Anmeldung auf mobilen Geräten
...
### <a name="how-to-restrict-teams-sign-in-on-desktop-devices"></a>Einschränken der Teams-Anmeldung auf Desktopgeräten

Microsoft Teams-Apps unter Windows und macOS erhalten Unterstützung für Geräterichtlinien, die die Anmeldung bei Ihrer Organisation einschränken. Die Richtlinien können über herkömmliche Geräteverwaltungslösungen wie MDM (Mobile Device Management) oder GPO (Group Policy Object) festgelegt werden.
...
### <a name="global-sign-in-and-microsoft-teams"></a>Globale Anmeldung und Microsoft Teams
...
## <a name="urls-and-ip-address-ranges-for-microsoft-teams"></a>URLs und IP-Adressbereiche für Microsoft Teams
