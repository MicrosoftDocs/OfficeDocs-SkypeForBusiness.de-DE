...
ms.locfileid: "50055690"
---
# <a name="virtual-visits-with-teams---integration-into-ehr"></a>Virtuelle Besuche mit Teams – Integration in EHR

Microsoft Teams Electronic Health Record (EHR) Connector erleichtert Den Kliniken die Einführung eines virtuellen Patientenbesuchs oder einer Beratung mit einem anderen Anbieter in Teams direkt über das EHR-System. Microsoft Teams, das auf der Microsoft 365-Cloud aufgebaut ist, ermöglicht eine einfache, sichere Zusammenarbeit und Kommunikation mit Chat-, Video-, Sprach- und Gesundheitstools in einem einzigen Hub, der die Einhaltung von HIPAA- und HITECH-Zertifizierungen unterstützt.
Die Kommunikations- und Zusammenarbeitsplattform von Teams macht es den Kliniken leicht, die Unübersichtlichkeit fragmentierter Systeme zu überschneiden, sodass sie Zeit für die bestmögliche Pflege auf verwenden können. Microsoft Teams Electronic Health Record (EHR) Connector kann:
- Starten Sie virtuelle Besuche von Teams sowohl über Anbieter- als auch Patientenportale.
- Schreiben Sie in die Metadaten der EHR zurück, um Verbindungs- und Trennungsereignisse zu aktivieren, um die automatische Überwachung und Datensatzspeicherung zu ermöglichen.
...
- Aktives Abonnement für Microsoft Cloud im Gesundheitswesen oder Abonnement des eigenständigen Microsoft Teams EHR Connector-Angebots.
...
> [!Note]
> Fordern Sie Ihren technischen Experten für Epe an, den Epic-Microsoft Teams Telehealth Integration Guide zur Verfügung zu stellen, der auf dem Marketplace für Epe verfügbar ist.
...
Das Konfigurieren Ihrer Gesundheitsorganisation für virtuelle Besuche mit Microsoft Teams beginnt mit dem Starten des Konfigurationsportals von EHR Connector. Sie konfigurieren eine oder mehrere Organisationen, um die Integration zu testen. Konfigurieren Sie die Test- und Produktions-URL im Konfigurationsportal. Testen Sie die Integration aus der Testumgebung von Epe, bevor Sie in die Produktion übersenden.
  
- Konfigurations-URL des EHR-Connectors: [https://ehrconnector.teams.microsoft.com](https://ehrconnector.teams.microsoft.com)

Der Microsoft 365-Administrator und Epischer Kundenanalyst aus Ihrer Organisation muss die Informations- und Integrationsschritte im Konfigurationsportal ausführen. Wenden Sie sich für epische Konfigurationsschritte an die Ihrem Unternehmen zugewiesene Technische Spezialressource "Epische Technik".
...
Um diesen Schritt ausführen zu können, muss der Microsoft 365-Administrator eine gültige FSCHI (Fast Health Interoperability Resources)-Basis-URL von Ihrem epischen technischen Experten und den Benutzernamen des Epischen Kundenanalysten erhalten, der die Konfiguration genehmigen wird. Der Microsoft 365-Administrator muss die Connectorkonfigurationsseite starten und sich mit den Microsoft-Anmeldeinformationen anmelden, um den Konfigurationsprozess zu starten.
...
- Der Name der genehmigenden Person für die Konfiguration ist der Name des epischen Kundenanalysten, der für die Genehmigung der Konfiguration im nächsten Schritt zuständig ist. Der Epische Kundenanalyst ist eine Person in Ihrer Organisation mit Anmeldezugriff auf Epen.

  ![Der Name der genehmigende Konfigurations genehmigende Benutzer wird aus einer Liste im EHR-Connector ausgewählt.](../../media/teams-ehr-connector.png)

### <a name="approve-or-view-configuration"></a>[Genehmigen oder Anzeigen der Konfiguration](#approve-or-view-configuration)

Der epische Kundenanalyst für Ihre Gesundheitsorganisation, der als genehmigende Benutzer hinzugefügt wurde, muss jetzt die gleiche URL für EHR Connector aus dem vorherigen Schritt verwenden, um sich mit den Microsoft 365-Anmeldeinformationen anmelden. Nach erfolgreicher Überprüfung wird die genehmigende Benutzer aufgefordert, sich mit ihren epischen Anmeldeinformationen zur Überprüfung der Epischen Organisation anmelden.

> [!Note]
> Der Microsoft 365-Administrator und epische Kundenanalyst in Ihrer Organisation kann dieselbe Person sein. Fügen Sie in diesem Fall Ihren eigenen Benutzernamen als genehmigende Benutzer hinzu. Sie müssen sich weiterhin bei "Episch" anmelden, um Ihren Zugriff zu überprüfen. Die epische Anmeldung wird nur zum Überprüfen Ihrer FSCHI-Basis-URL verwendet. Microsoft kann keine Anmeldeinformationen speichern oder mit dieser Anmeldung auf die Daten von EHR zugreifen.

  ![Überprüfen und genehmigen Sie die Konfiguration der Anmeldeinformationen.](../../media/approve-view-configuration.png)

Nach einer erfolgreichen Epischen Anmeldung muss der Epische **Kundenanalyst** die Konfiguration genehmigen. Wenn die Konfiguration nicht korrekt ist, kann der Microsoft 365-Administrator die ursprünglichen Konfigurationen ändern, indem er sich erneut beim Microsoft EHR-Connectorportal anmelden. 
...
Wenn die Konfigurationsinformationen vom Epischen Administrator genehmigt wurden, erhalten Sie Integrationsaufzeichnungen für den Patienten und den Anbieterstart. Diese Einträge sind erforderlich, um die Konfiguration für den virtuellen Besuch in Episch abschließen zu können. Weitere Details finden Sie Epic-Microsoft Teams Telehealth Integration.
...
Nach Abschluss der Schritte für den EHR Connector und der Konfiguration von Epen ist Ihre Organisation bereit, Videobesuche mit Microsoft Teams zu unterstützen.
...
Gesundheitswesensanbieter in Ihrer Organisation können auch an virtuellen Besuchen mit Microsoft Teams aus ihren Epischen Anbieteranwendungen (Hyperspace, Cubeku, Canto) teilnehmen. Die **Schaltfläche "Virtuellen Besuch** starten" ist in den Anbieterfluss eingebettet.
...
- Patienten können über moderne Webbrowser auf desktop- und mobilen Geräten ohne App-Installation an virtuellen Besuchen teilnehmen.
