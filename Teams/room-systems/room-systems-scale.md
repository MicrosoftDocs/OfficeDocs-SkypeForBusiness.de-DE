...
### <a name="configure-distribution-points-to-support-pxe-and-multicast"></a><span data-ttu-id="8ccd2-142">Konfigurieren von Verteilungspunkten zur Unterstützung von PXE und Multicast</span><span class="sxs-lookup"><span data-stu-id="8ccd2-142">Configure distribution points to support PXE and multicast</span></span>
...
3.  <span data-ttu-id="8ccd2-155">Wählen Sie die Registerkarte **Netzwerkzugriffskonto** aus. richten Sie mindestens ein Konto ein, und wählen Sie dann **OK**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-155">Select the **Network Access Account** tab. Set up one or more accounts, and then select **OK**.</span></span>
...
5.  <span data-ttu-id="8ccd2-166">*Optional:* Um die Bereitstellungsumgebung anzupassen, wählen Sie die Registerkarte **Anpassung** aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-166">*Optional:* To customize the deployment experience, select the **Customization** tab.</span></span>
    ...
    -   <span data-ttu-id="8ccd2-169">Sie können auch ein benutzerdefiniertes Hintergrundbild angeben, das während der Bereitstellung angezeigt werden soll.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-169">You can also specify a custom background image to be displayed during the deployment.</span></span> <span data-ttu-id="8ccd2-170">Wenn Sie ein Bild festlegen möchten, aktivieren **Sie die Option geben Sie die benutzerdefinierte Hintergrund Bilddatei an (UNC-Pfad** , und wählen Sie den Hintergrund aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-170">To set an image, enable **Specify the custom background image file (UNC path** and select your background.</span></span>
...
<span data-ttu-id="8ccd2-180">Configuration Manager erfordert eine Reihe von Paketen zum Bereitstellen und Konfigurieren der Microsoft Teams rooms-Einheiten.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-180">Configuration Manager requires a number of packages to deploy and configure the Microsoft Teams Rooms units.</span></span>
...
5. <span data-ttu-id="8ccd2-244">Wählen Sie **Programm nicht erstellen**aus, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-244">Select **Do not create a program**, and then select **Next**.</span></span>
...
1.  <span data-ttu-id="8ccd2-268">Kopieren Sie das Stammzertifikat in den Ordner **SRS v2 – Stammzertifikat Paket** .</span><span class="sxs-lookup"><span data-stu-id="8ccd2-268">Copy the root certificate into the **SRS v2 – Root Certificate Package** folder.</span></span>
...
4.  <span data-ttu-id="8ccd2-282">Geben Sie die folgenden Informationen ein, um das Paket zu erstellen:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-282">Enter the following information to create the package:</span></span>
...
### <a name="create-the-sysprep-package"></a><span data-ttu-id="8ccd2-303">Erstellen des Sysprep-Pakets</span><span class="sxs-lookup"><span data-stu-id="8ccd2-303">Create the Sysprep package</span></span>
...
1.  <span data-ttu-id="8ccd2-329">Laden Sie die neuesten Treiber und Firmware herunter.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-329">Download the latest drivers and firmware.</span></span>
...
10. <span data-ttu-id="8ccd2-341">Wählen Sie **Schließen**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-341">Select **Close**.</span></span>
...
3.  <span data-ttu-id="8ccd2-353">Wählen Sie **Programm nicht erstellen**aus, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-353">Select **Do not create a program**, and then select **Next**.</span></span>
...
2.  <span data-ttu-id="8ccd2-365">Treiberpakete verteilen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-365">Distribute driver packages.</span></span>
    ...
    2.  <span data-ttu-id="8ccd2-368">Überprüfen Sie die Liste der Pakete, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-368">Review the list of packages, and then select **Next**.</span></span>
...
## <a name="configuration-manager-task-sequences"></a><span data-ttu-id="8ccd2-380">Tasksequenzen für Configuration Manager</span><span class="sxs-lookup"><span data-stu-id="8ccd2-380">Configuration Manager task sequences</span></span>
...
6.  <span data-ttu-id="8ccd2-390">Wählen Sie **Schließen**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-390">Select **Close**.</span></span>

### <a name="validate-that-the-reference-packages-are-correctly-linked-to-each-task-sequence-step"></a><span data-ttu-id="8ccd2-391">Überprüfen Sie, ob die Verweis Pakete ordnungsgemäß mit jedem Tasksequenzschritt verknüpft sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-391">Validate that the reference packages are correctly linked to each task sequence step.</span></span>

1. <span data-ttu-id="8ccd2-392">Wählen Sie die importierte Tasksequenz aus, und wählen Sie dann **Bearbeiten**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-392">Select the imported task sequence, and then select **Edit**.</span></span>

    <span data-ttu-id="8ccd2-393">Der Task Sequenz-Editor wird geöffnet und zeigt jeden sequenziellen Schritt an, den Sie für die Bereitstellung und Konfiguration einer Microsoft Teams rooms-Einheit benötigen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-393">The Task Sequence Editor opens and displays each sequential step that you need to deploy and configure a Microsoft Teams Rooms unit.</span></span>

2. <span data-ttu-id="8ccd2-394">Durchlaufen Sie die einzelnen Schritte, und führen Sie die empfohlenen Updates aus:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-394">Walk through each step and complete the recommended updates:</span></span>

   1. <span data-ttu-id="8ccd2-395">**Neustart in Windows PE**: dieser Schritt wird neu gestartet und dann der Computer in Windows PXE gestartet.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-395">**Restart in Windows PE**: This step restarts and then boots the computer into Windows PXE.</span></span> <span data-ttu-id="8ccd2-396">Für diesen Schritt sind keine Änderungen erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-396">No changes are required for this step.</span></span>

   2. <span data-ttu-id="8ccd2-397">**Partition Disk 0 – UEFI**: dieser Schritt löscht die Datenträgerkonfiguration und erstellt Partitionen basierend auf den konfigurierten Einstellungen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-397">**Partition Disk 0 – UEFI**: This step wipes the disk configuration and creates partitions based on the configured settings.</span></span> <span data-ttu-id="8ccd2-398">Wir empfehlen, dass Sie an diesem Schritt keine Änderungen vornehmen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-398">We recommend that you not make any changes to this step.</span></span>

   3. <span data-ttu-id="8ccd2-399">**SRS-Computer Name einrichten**: dieser Schritt umfasst eine HTML-Anwendung, um eine Benutzeroberfläche bereitzustellen, um einen Computer Namen für die Einheit Microsoft Teams rooms während der Bereitstellung zu definieren.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-399">**Set SRS Computer Name**: This step includes an HTML application to provide a UI to set a computer name for the Microsoft Teams Rooms unit during the deployment.</span></span>
      -  <span data-ttu-id="8ccd2-400">Dies ist ein optionaler Schritt, der aber nur deaktiviert werden kann, wenn Sie die Computer Namensgebung in einem anderen Prozess verwalten möchten.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-400">This is an optional step, but it can only be disabled if you want to manage computer naming through an alternate process.</span></span>
      -  <span data-ttu-id="8ccd2-401">Überprüfen Sie, ob das Paket **SRS v2-Set-SRSComputerName** ausgewählt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-401">Verify that the **SRS v2 - Set-SRSComputerName** package is selected.</span></span> <span data-ttu-id="8ccd2-402">Wenn dies nicht der Fall ist, wechseln Sie zum Paket, und wählen Sie es aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-402">If it isn’t, browse to the package and select it.</span></span>

   4. <span data-ttu-id="8ccd2-403">**Betriebssystem anwenden**: dieser Schritt gibt das Betriebssystemabbild an, das bereitgestellt werden soll, und die zu verwendende Antwortdatei für die unbeaufsichtigte Sysprep.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-403">**Apply Operating System**: This step specifies the operating system image to be deployed and the unattended Sysprep answer file to use.</span></span>
      -  <span data-ttu-id="8ccd2-404">Überprüfen Sie, ob die richtige Abbilddatei für das Windows 10 Enterprise-Betriebssystem ausgewählt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-404">Verify that the correct Windows 10 Enterprise operating system image file is selected.</span></span>
      -  <span data-ttu-id="8ccd2-405">Stellen Sie sicher, dass die **Verwendung einer Antwortdatei für eine unbeaufsichtigte oder Sysprep für eine benutzerdefinierte Installation** aktiviert ist und das **SRS v2-Sysprep-Paket** ausgewählt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-405">Verify that **Use an unattended or Sysprep answer file for a custom installation** is enabled, and the **SRS v2 - Sysprep Package** is selected.</span></span> <span data-ttu-id="8ccd2-406">Stellen Sie außerdem sicher, dass der **Dateiname** auf **Unattend. XML**festgesetzt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-406">Also ensure that **File Name** is set to **unattend.xml**.</span></span>

   5. <span data-ttu-id="8ccd2-407">**Windows-Einstellungen übernehmen**: dieser Schritt sammelt Informationen zur Windows-Installation.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-407">**Apply Windows Settings**: This step gathers information about the Windows installation.</span></span>
      -  <span data-ttu-id="8ccd2-408">Stellen Sie Lizenzierungs-und Registrierungsinformationen bereit, einschließlich des Product Keys, des Kennworts für das lokale Administratorkonto und der Zeitzone (je nach Ihren Anforderungen).</span><span class="sxs-lookup"><span data-stu-id="8ccd2-408">Provide licensing and registration information including the product key, local administrator account password, and time zone (depending on your needs).</span></span>

   6. <span data-ttu-id="8ccd2-409">**Netzwerkeinstellungen übernehmen**: mit diesem Schritt können Sie eine Arbeitsgruppe oder einen Active Directory-Domänennamen und eine Organisationseinheit angeben.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-409">**Apply Network Settings**: This step allows you to specify a workgroup or Active Directory domain name and organizational unit.</span></span>
      > [!NOTE]
      > <span data-ttu-id="8ccd2-410">Informationen zu empfohlenen Aktionen, die Sie beim Bereitstellen von Microsoft Teams rooms-Einheiten als Mitglieder einer Actve-Verzeichnisdomäne ausführen müssen, finden Sie unter [Überlegungen zu Skype Room System Domain Joining](domain-joining-considerations.md) .</span><span class="sxs-lookup"><span data-stu-id="8ccd2-410">See [Skype Room System domain joining considerations](domain-joining-considerations.md) for recommended actions you need to take in deploying Microsoft Teams Rooms units as members of an Actve Directory domain.</span></span>
   7. <span data-ttu-id="8ccd2-411">**Anwenden von Treibern:** Dieser Schritt und seine unter Schritte werden verwendet, um anwendbare Gerätetreiber und Firmware basierend auf dem von Ihnen verwendeten Surface pro-Modell bereitzustellen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-411">**Apply Drivers:** This step and its sub-steps are used to deploy applicable device drivers and firmware based on the Surface Pro model you have.</span></span> <span data-ttu-id="8ccd2-412">Aktualisieren Sie die einzelnen Schritte, um das entsprechende Treiberpaket anzugeben, das dieser Bereitstellung zugeordnet ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-412">Update each step to specify the relevant driver package associated with this deployment.</span></span>
      -   <span data-ttu-id="8ccd2-413">Jedes Treiberpaket ist so konfiguriert, dass es die Windows-Verwaltungsinstrumentation (WMI)-Filter nutzt, um relevante Treiber und Firmware basierend auf dem Surface pro-Hersteller und-Modell bereitzustellen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-413">Each driver package is configured to leverage Windows Management Instrumentation (WMI) filters to deploy relevant drivers and firmware based on the Surface Pro make and model.</span></span>
      -   <span data-ttu-id="8ccd2-414">Wir empfehlen dringend, dass Sie die Konfiguration dieser Treiber nicht ändern, da die Bereitstellung andernfalls möglicherweise fehlschlägt.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-414">We highly recommend that you not alter the configuration of these drivers, otherwise deployment might fail.</span></span>

   8. <span data-ttu-id="8ccd2-415">**Einrichten von Windows und Configuration Manager**: dieser Schritt stellt den Configuration Manager-Client bereit und konfiguriert ihn.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-415">**Set up Windows and Configuration Manager**: This step deploys and configures the Configuration Manager client.</span></span> <span data-ttu-id="8ccd2-416">Aktualisieren Sie diesen Schritt, um das integrierte Configuration Manager-Client Paket anzugeben.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-416">Update this step to specify the built-in Configuration Manager Client Package.</span></span>

   9. <span data-ttu-id="8ccd2-417">**Installieren des Stammzertifikats**: mit diesem Schritt wird das Stammzertifikat für nicht-Domänen verbundene Geräte verteilt und ist daher standardmäßig optional und deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-417">**Install Root Certificate**: This step distributes the root certificate for non–domain-joined devices, and therefore is optional and disabled by default.</span></span>
      -   <span data-ttu-id="8ccd2-418">Aktivieren Sie diesen Schritt, wenn Sie ein Stammzertifikat für die Microsoft Teams-Zimmereinheiten bereitstellen müssen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-418">Enable this step if you need to deploy a root certificate to the Microsoft Teams Rooms units.</span></span>
      -   <span data-ttu-id="8ccd2-419">Wenn Sie diesen Schritt ausführen müssen, stellen Sie sicher, dass das **SRS v2 – Stammzertifikat Paket** und die Deaktivierung der **64-Bit-Dateisystem Umleitung** ausgewählt sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-419">If you do need to perform this step, verify that the **SRS v2 – Root Certificate Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   10. <span data-ttu-id="8ccd2-420">**Installieren und Konfigurieren des Überwachungs-Agents**: mit diesem Schritt wird die 64-Bit-Version des Microsoft Azure Monitor-Agents installiert und der Agent so konfiguriert, dass eine Verbindung mit Ihrem Protokollanalyse Arbeitsbereich hergestellt wird.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-420">**Install and Configure Monitoring Agent**: This step installs the 64-bit version of the Microsoft Azure Monitor agent and configures the agent to connect to your Log Analytics workspace.</span></span>
       -   <span data-ttu-id="8ccd2-421">Dieser Schritt ist standardmäßig deaktiviert.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-421">This step is disabled by default.</span></span> <span data-ttu-id="8ccd2-422">Aktivieren Sie diesen Schritt nur, wenn Sie den Überwachungs-Agent verwenden möchten, um den Status Ihrer Microsoft Teams-Zimmereinheiten zu überwachen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-422">Enable this step only if you’re going to use the Monitoring Agent to monitor the health of your Microsoft Teams Rooms units.</span></span>
       -   <span data-ttu-id="8ccd2-423">Bearbeiten Sie diesen Schritt, und aktualisieren Sie die Befehlszeilenparameter, um die **Arbeitsbereichs-ID** und den **Arbeitsbereichs Schlüssel**anzugeben.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-423">Edit this step and update the command-line parameters to specify your **Workspace ID** and **Workspace Key**.</span></span>
       -   <span data-ttu-id="8ccd2-424">Weitere Informationen zum Abrufen der Arbeitsbereichs-ID der Operations Management Suite und des Primärschlüssels finden Sie unter [Konfigurieren von Testgeräten für die Azure-Überwachung](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) .</span><span class="sxs-lookup"><span data-stu-id="8ccd2-424">See [Configure test devices for Azure Monitoring](azure-monitor-deploy.md#configure-test-devices-for-azure-monitoring) for more information about obtaining the Operations Management Suite Workspace ID and the primary key.</span></span>
       -   <span data-ttu-id="8ccd2-425">Vergewissern Sie sich, dass das **SRS v2 – Microsoft Monitoring Agent-Paket** und die Deaktivierung der **64-Bit-Dateisystem Umleitung** ausgewählt sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-425">Verify that the **SRS v2 – Microsoft Monitoring Agent Package** and **Disable 64-bit file system redirection** are selected.</span></span>
       -   <span data-ttu-id="8ccd2-426">Weitere Informationen zum Überwachen der Integrität Ihrer Microsoft Teams rooms-Bereitstellung finden Sie unter [Planen der Microsoft Teams rooms-Verwaltung mit Azure Monitor](azure-monitor-plan.md), [Bereitstellen der Microsoft Teams rooms-Verwaltung mit Azure Monitor](azure-monitor-deploy.md) und [Verwalten von Microsoft Teams Räume Geräte mit Azure Monitor](azure-monitor-manage.md).</span><span class="sxs-lookup"><span data-stu-id="8ccd2-426">For more information about monitoring the health of your Microsoft Teams Rooms deployment, see [Plan Microsoft Teams Rooms management with Azure Monitor](azure-monitor-plan.md), [Deploy Microsoft Teams Rooms management with Azure Monitor](azure-monitor-deploy.md) and [Manage Microsoft Teams Rooms devices with Azure Monitor](azure-monitor-manage.md).</span></span>

   11. <span data-ttu-id="8ccd2-427">**Kopieren von SRS v2-Konfigurationsdateien**: dieser Schritt kopiert die erforderlichen Setup-und Konfigurationsdateien aus dem Microsoft Teams rooms Deployment Kit auf die lokale Festplatte.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-427">**Copy SRS v2 Configuration Files**: This step copies the required setup and configuration files from the Microsoft Teams Rooms deployment kit to the local hard drive.</span></span> <span data-ttu-id="8ccd2-428">Für diesen Schritt sind keine Anpassungen erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-428">No customization is required for this step.</span></span>
       -   <span data-ttu-id="8ccd2-429">Vergewissern Sie sich, dass das **SRS v2-SRS-Anwendungspaket** und die Deaktivierung der **64-Bit-Dateisystem Umleitung** ausgewählt sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-429">Verify that the **SRS v2 – SRS Application Package** and **Disable 64-bit file system redirection** are selected.</span></span>

   12. <span data-ttu-id="8ccd2-430">**Installieren-SRSv2-OS-Updates**: in diesem Schritt werden alle obligatorischen Betriebssystemupdates bereitgestellt, die für die Microsoft Teams rooms-Bereitstellung erforderlich sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-430">**Install-SRSv2-OS-Updates**: This step deploys any mandatory operating system updates required with the Microsoft Teams Rooms deployment.</span></span> <span data-ttu-id="8ccd2-431">Gehen Sie wie folgt vor:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-431">Do the following:</span></span>
       -   <span data-ttu-id="8ccd2-432">Aktivieren Sie das Kontrollkästchen [Konfigurieren einer Microsoft Teams rooms-Konsole](console.md) , um festzustellen, welche Updates erforderlich sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-432">Check [Configure a Microsoft Teams Rooms console](console.md) to see which updates are required.</span></span>
       -   <span data-ttu-id="8ccd2-433">Überprüfen Sie, ob Ihr **SRS v2 – OS-Updatepaket** alle erforderlichen Updates enthält.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-433">Verify that your **SRS v2 – OS Updates Package** includes all the required updates.</span></span>
       -   <span data-ttu-id="8ccd2-434">Überprüfen Sie, ob das **Paket SRS v2 – Betriebssystem Updates** ausgewählt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-434">Verify that the **SRS v2 – OS Updates Package** is selected.</span></span>
       -   <span data-ttu-id="8ccd2-435">Überprüfen Sie, ob die PowerShell-Ausführungsrichtlinie auf **Bypass**festgesetzt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-435">Verify that the PowerShell execution policy is set to **Bypass**.</span></span>

   13. <span data-ttu-id="8ccd2-436">**Computer neu starten**: mit diesem Schritt wird der Computer neu gestartet, nachdem die obligatorischen Betriebssystemupdates installiert wurden.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-436">**Restart Computer**: This step reboots the computer after the mandatory operating system updates are installed.</span></span> <span data-ttu-id="8ccd2-437">Für diesen Schritt sind keine Anpassungen erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-437">No customization is required for this step.</span></span>

   14. <span data-ttu-id="8ccd2-438">**Konfigurieren von Windows-Komponenten**: in diesem Schritt werden die erforderlichen Windows-Features konfiguriert.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-438">**Configure Windows Components**: This step configures the required Windows features.</span></span> <span data-ttu-id="8ccd2-439">Für diesen Schritt sind keine Anpassungen erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-439">No customization is required for this step.</span></span>

   15. <span data-ttu-id="8ccd2-440">**Computer neu starten**: mit diesem Schritt wird der Computer neu gestartet, nachdem die Windows-Features konfiguriert wurden.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-440">**Restart Computer**: This step reboots the computer after the Windows features are configured.</span></span> <span data-ttu-id="8ccd2-441">Für diesen Schritt sind keine Anpassungen erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-441">No customization is required for this step.</span></span>

   16. <span data-ttu-id="8ccd2-442">**Lokalen Skype-Nutzer hinzufügen**: mit diesem Schritt wird das lokale Skype-Konto erstellt, das für die automatische Anmeldung bei Windows und das Starten der Microsoft Teams rooms-Anwendung verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-442">**Add Local Skype User**: This step creates the local Skype account used to automatically sign in to Windows and start the Microsoft Teams Rooms application.</span></span> <span data-ttu-id="8ccd2-443">Diesem Schritt ist kein Softwarepaket zugeordnet, und es ist keine Anpassung erforderlich.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-443">This step doesn’t have any software package associated with it, and no customization is required for it.</span></span>

   17. <span data-ttu-id="8ccd2-444">**Einrichten und Konfigurieren von SRS-Anwendungen**: dieser Schritt konfiguriert die Anwendungsinstallation von Microsoft Teams rooms für den nächsten Start des Betriebssystems.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-444">**Set up and configure SRS application**: This step configures the Microsoft Teams Rooms application installation for the next boot of the operating system.</span></span>
       -   <span data-ttu-id="8ccd2-445">Vergewissern Sie sich, dass das SRS **v2 – Setup-Paket** für die SRS-Konfiguration und die **64-Bit-Dateisystem Umleitung deaktiviert** sind.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-445">Verify that the **SRS v2 – Configure SRS Setup Package** and **Disable 64-bit file system redirection** are selected.</span></span>

> [!IMPORTANT]
> <span data-ttu-id="8ccd2-446">Es ist sehr wichtig, dass die Tasksequenzschritte in der angegebenen Reihenfolge ausgeführt werden müssen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-446">It is very important that the task sequence steps must be in the provided order.</span></span> <span data-ttu-id="8ccd2-447">Wenn Sie die Reihenfolge der Schritte ändern oder zusätzliche Schritte konfigurieren, kann dies die Bereitstellung unterbrechen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-447">Modifying the order of steps, or configuring additional steps might break the deployment.</span></span>
>
> <span data-ttu-id="8ccd2-448">Das **Einrichten und Konfigurieren des SRS-Anwendungs** Schritts muss der letzte Schritt in der Tasksequenz sein, da andernfalls möglicherweise die Bereitstellung fehlschlägt.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-448">**Set up and configure SRS application** step must be the last step in the task sequence, otherwise the deployment might fail.</span></span>

### <a name="create-deployment-for-the-task-sequence"></a><span data-ttu-id="8ccd2-449">Erstellen einer Bereitstellung für die Tasksequenz</span><span class="sxs-lookup"><span data-stu-id="8ccd2-449">Create deployment for the task sequence</span></span>

1. <span data-ttu-id="8ccd2-450">Wählen Sie die Tasksequenz aus, und wählen Sie **Bereitstellen**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-450">Select the task sequence, and then select **Deploy**.</span></span>

2. <span data-ttu-id="8ccd2-451">Wählen Sie **Durchsuchen** aus, um die Zielsammlung für die Bereitstellung auszuwählen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-451">Select **Browse** to select target collection for deployment.</span></span>

3. <span data-ttu-id="8ccd2-452">Wählen Sie **alle unbekannten Computer** aus, und wählen Sie dann **OK**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-452">Select **All Unknown Computers** and then select **OK**.</span></span>

4. <span data-ttu-id="8ccd2-453">Wählen Sie **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-453">Select **Next**.</span></span>

5. <span data-ttu-id="8ccd2-454">Wählen Sie in der \*\*\*\* Dropdownliste Purpose **available (verfügbar** ) aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-454">Select **Available** on the **Purpose** drop down list.</span></span>

6. <span data-ttu-id="8ccd2-455">Wählen Sie **nur Medien und PXE** in der Liste in der **folgenden Liste verfügbar machen** aus, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-455">Select **Only Media and PXE** in the **Make available to the following** list, and then select **Next**.</span></span>
   > [!WARNING]
   > <span data-ttu-id="8ccd2-456">Es ist sehr wichtig, \*\*\*\* dass Purpose auf **available**festgesetzt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-456">It is very important that **Purpose** is set to **Available**.</span></span> <span data-ttu-id="8ccd2-457">Stellen Sie sicher, dass der **Zweck** **nicht** auf **erforderlich**festgesetzt ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-457">Make sure that the **Purpose** is **NOT** set to **Required**.</span></span> <span data-ttu-id="8ccd2-458">Stellen Sie außerdem sicher, dass Sie **nur Medien und PXE** in der **folgenden verfügbar machen**auswählen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-458">Also make sure that you select **Only Media and PXE** in the **Make available to the following**.</span></span>
   >
   > <span data-ttu-id="8ccd2-459">Wenn diese Werte auf etwas anderes festgelegt werden, kann dies dazu führen, dass alle Computer beim Booten das Bereitstellungs Bild von Microsoft Teams rooms erhalten.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-459">Setting these values to something else might cause all computers to get the Microsoft Teams Rooms deployment image when booted.</span></span>
7. <span data-ttu-id="8ccd2-460">Geben Sie keinen Zeitplan an, und wählen Sie **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-460">Do not specify any schedule and select **Next**.</span></span>

8. <span data-ttu-id="8ccd2-461">Ändern Sie im Abschnitt **Benutzerfreundlichkeit** nichts, und wählen Sie **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-461">Do not change anything within the **User Experience** section and select **Next**.</span></span>

9. <span data-ttu-id="8ccd2-462">Ändern Sie im Abschnitt **Benachrichtigungen** nichts, und wählen Sie **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-462">Do not change anything within the **Alerts** section and select **Next**.</span></span>

10. <span data-ttu-id="8ccd2-463">Ändern Sie im Abschnitt Verteilungs **Punkte** nichts, und wählen Sie **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-463">Do not change anything within the **Distribution Points** section and select **Next**.</span></span>

11. <span data-ttu-id="8ccd2-464">Bestätigen Sie die Einstellungen, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-464">Confirm the settings and then select **Next**.</span></span>

12. <span data-ttu-id="8ccd2-465">Wählen Sie **Schließen**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-465">Select **Close**.</span></span>

<a name="validate-and-troubleshoot-the-solution"></a><span data-ttu-id="8ccd2-466">Überprüfen und Problembehandlung der Lösung</span><span class="sxs-lookup"><span data-stu-id="8ccd2-466">Validate and troubleshoot the solution</span></span>
--------------------------------------

<span data-ttu-id="8ccd2-467">Nachdem Sie die Tasksequenzen des System Center Configuration Manager abgeschlossen haben, müssen Sie einen Testlauf durchführen, um zu überprüfen, ob die Tasksequenz Microsoft Teams-Zimmereinheiten bereitstellen und konfigurieren kann.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-467">After you’ve completed the System Center Configuration Manager task sequences, you’ll need to perform a test run to validate that the task sequence can deploy and configure Microsoft Teams Rooms units.</span></span>

1.  <span data-ttu-id="8ccd2-468">Verbinden Sie das Test Gerät mit dem kabelgebundenen Netzwerk, indem Sie einen der unterstützten Ethernet-Adapter verwenden oder die Surface-Docking-Station verwenden.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-468">Connect the test device to the wired network by using one of the supported Ethernet adapters or using the Surface dock.</span></span> <span data-ttu-id="8ccd2-469">Wenn die PXE-Start Funktionalität für Ihre Umgebung nicht konfiguriert wurde, können Sie das Startabbild auf dem USB-Stick verwenden, den [Sie zuvor erstellt](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media) haben, um von USB zu booten und mit Configuration Manager zu verbinden.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-469">If PXE boot functionality hasn’t been configured for your environment, you can use the boot image on the USB flash drive [that you created earlier](https://docs.microsoft.com/sccm/osd/deploy-use/create-bootable-media) to boot from USB and connect to Configuration Manager.</span></span>

2.  <span data-ttu-id="8ccd2-470">Zugreifen auf die Firmware und Initiieren des PXE-Starts:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-470">Access the firmware and initiate PXE boot:</span></span>

    1.  <span data-ttu-id="8ccd2-471">Stellen Sie sicher, dass das Surface-Gerät ausgeschaltet ist.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-471">Ensure the Surface device is powered off.</span></span>

    2.  <span data-ttu-id="8ccd2-472">Drücken Sie die **Lautstärke** Taste und halten Sie sie gedrückt.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-472">Press and hold the **Volume Up** button.</span></span>

    3.  <span data-ttu-id="8ccd2-473">Drücken Sie die **Power** -Taste, und lassen Sie sie los.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-473">Press and release the **Power** button.</span></span>

    4.  <span data-ttu-id="8ccd2-474">Nachdem das Gerät gestartet wurde, lassen Sie die **Lautstärke** Taste los.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-474">After the device begins to boot, release the **Volume Up** button.</span></span>

    5.  <span data-ttu-id="8ccd2-475">Wählen Sie **Startkonfiguration**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-475">Select **Boot configuration**.</span></span>

    6.  <span data-ttu-id="8ccd2-476">Führen Sie einen der folgenden Schritte aus:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-476">Do one of the following:</span></span>

        -   <span data-ttu-id="8ccd2-477">Wählen Sie **PXE-Start**aus, und ziehen Sie den Mauszeiger an den Anfang der Liste.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-477">Select **PXE boot**, and drag it to the top of the list.</span></span> <span data-ttu-id="8ccd2-478">Sie können aber auch nach links auf dem Netzwerkadapter wischen, um sofort auf das Gerät zu booten.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-478">Alternatively, you can swipe left on the network adapter to boot to the device immediately.</span></span> <span data-ttu-id="8ccd2-479">Dies wirkt sich nicht auf die Startreihenfolge aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-479">This won’t affect the boot order.</span></span>
        -   <span data-ttu-id="8ccd2-480">Wählen Sie das USB-Flashlaufwerk aus, das die Startmedien enthält.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-480">Select the USB flash drive that holds the boot media.</span></span>

3.  <span data-ttu-id="8ccd2-481">Wählen Sie **Beenden**und dann **jetzt neu starten**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-481">Select **Exit**, and then select **Restart Now**.</span></span>

4.  <span data-ttu-id="8ccd2-482">Wenn Sie dazu aufgefordert werden, wählen Sie für den Netzwerkstart Dienst **Enter** aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-482">When prompted, select **Enter** for network boot service.</span></span>

5.  <span data-ttu-id="8ccd2-483">Windows PE wird in den Arbeitsspeicher geladen, und der Task Sequenz-Assistent wird gestartet.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-483">Windows PE will load into memory, and the Task Sequence Wizard will start.</span></span> <span data-ttu-id="8ccd2-484">Wählen Sie **weiter** aus, um fortzufahren.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-484">Select **Next** to continue.</span></span>

6.  <span data-ttu-id="8ccd2-485">Wählen Sie die zuvor importierte Tasksequenz aus, und wählen Sie dann **weiter**aus.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-485">Select the task sequence that you imported earlier, and then select **Next**.</span></span>

7.  <span data-ttu-id="8ccd2-486">Nachdem die Datenträgerkonfiguration angewendet wurde, werden Sie aufgefordert, einen Computernamen für das Gerät anzugeben.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-486">After the disk configuration is applied, you’ll be prompted to specify a computer name for the device.</span></span> <span data-ttu-id="8ccd2-487">Auf der Benutzeroberfläche wird ein empfohlener Computername basierend auf der Seriennummer des Surface pro-Geräts angezeigt.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-487">The user interface will display a recommended computer name based on the serial number of the Surface Pro device.</span></span> <span data-ttu-id="8ccd2-488">Sie können entweder den vorgeschlagenen Namen akzeptieren oder einen neuen Namen angeben.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-488">You can either accept the proposed name or specify a new one.</span></span> <span data-ttu-id="8ccd2-489">Folgen Sie den Anweisungen auf dem Bildschirm Computername-Zuordnung.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-489">Follow the instructions on the computer name assignment screen.</span></span> <span data-ttu-id="8ccd2-490">Wenn Sie **annehmen**auswählen, beginnt die Bereitstellung.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-490">When you select **Accept**, the deployment begins.</span></span>

8.  <span data-ttu-id="8ccd2-491">Der restliche Bereitstellungsprozess ist automatisch und fordert keine weitere Benutzereingabe.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-491">The rest of the deployment process is automatic and doesn’t ask for any more user input.</span></span>

9.  <span data-ttu-id="8ccd2-492">Nachdem die Bereitstellungstasksequenz die Konfiguration des Geräts abgeschlossen hat, wird der folgende Konfigurationsbildschirm angezeigt, in dem Sie aufgefordert werden, die Anwendungseinstellungen für Microsoft Teams Rooms zu konfigurieren.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-492">After the deployment task sequence finishes configuring the device, you’ll see the following configuration screen that asks you to configure the Microsoft Teams Rooms application settings.</span></span>

    ![Bildschirm ' Initial Setup ' für die Microsoft Teams rooms-Anwendung](../media/room-systems-scale-image2.png)

10.  <span data-ttu-id="8ccd2-494">Schließen Sie das Surface pro an die Microsoft Teams rooms-Konsole an, und konfigurieren Sie die Anwendungseinstellungen.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-494">Plug the Surface Pro into the Microsoft Teams Rooms console, and configure the application settings.</span></span>

11.  <span data-ttu-id="8ccd2-495">Überprüfen Sie, ob die in [Microsoft Teams](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) -Chatrooms aufgeführten Funktionen auf dem bereitgestellten Gerät funktionieren.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-495">Validate that the capabilities listed in [Microsoft Teams Rooms help](https://support.office.com/article/Skype-Room-Systems-version-2-help-e667f40e-5aab-40c1-bd68-611fe0002ba2) are working on the deployed device.</span></span>


<span data-ttu-id="8ccd2-496">Wenn Sie eine fehlerhafte Installation beheben möchten, überprüfen Sie die Datei **"smsts. log** , in der alle in einer Configuration Manager-Tasksequenz ausgeführten Schritte protokolliert werden.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-496">To troubleshoot a failed installation, check the **SMSTS.log** file, which logs all the steps executed in a Configuration Manager task sequence.</span></span>

<span data-ttu-id="8ccd2-497">Die Datei "" smsts. log "wird je nach Phase des Buildprozesses in einer Reihe von Pfaden gespeichert.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-497">The SMSTS.log file is stored on one of a number of paths, depending on the stage of the build process.</span></span> <span data-ttu-id="8ccd2-498">Überprüfen Sie die folgende Tabelle, um den Pfad zur Datei "" smsts. log "zu identifizieren.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-498">Check the following table to identify the path to the SMSTS.log file.</span></span>


| <span data-ttu-id="8ccd2-499">**Bereitstellungsphase**</span><span class="sxs-lookup"><span data-stu-id="8ccd2-499">**Deployment phase**</span></span>                                                            | <span data-ttu-id="8ccd2-500">**Task Sequenzprotokoll Pfad**</span><span class="sxs-lookup"><span data-stu-id="8ccd2-500">**Task sequence log path**</span></span>                         |
|---------------------------------------------------------------------------------|----------------------------------------------------|
| <span data-ttu-id="8ccd2-501">WinPE, vor dem Festplattenformat</span><span class="sxs-lookup"><span data-stu-id="8ccd2-501">WinPE, before HDD format</span></span>                                                        | <span data-ttu-id="8ccd2-502">X:\\Windows\\Temp\\smstslog\\"smsts. log</span><span class="sxs-lookup"><span data-stu-id="8ccd2-502">X:\\Windows\\Temp\\smstslog\\smsts.log</span></span>             |
| <span data-ttu-id="8ccd2-503">WinPE, nach Festplattenformat</span><span class="sxs-lookup"><span data-stu-id="8ccd2-503">WinPE, after HDD format</span></span>                                                         | <span data-ttu-id="8ccd2-504">C:\\_SMSTaskSequence\\Logs\\Smstslog\\"smsts. log</span><span class="sxs-lookup"><span data-stu-id="8ccd2-504">C:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="8ccd2-505">Betriebssystem, das vor der Installation des Configuration Manager-Agents bereitgestellt wurde</span><span class="sxs-lookup"><span data-stu-id="8ccd2-505">Operating system deployed, before the Configuration Manager agent was installed</span></span> | <span data-ttu-id="8ccd2-506">c:\\_SMSTaskSequence\\Logs\\Smstslog\\"smsts. log</span><span class="sxs-lookup"><span data-stu-id="8ccd2-506">c:\\_SMSTaskSequence\\Logs\\Smstslog\\smsts.log</span></span>    |
| <span data-ttu-id="8ccd2-507">Betriebssystem und bereitgestellter Configuration Manager-Agent</span><span class="sxs-lookup"><span data-stu-id="8ccd2-507">Operating system and the Configuration Manager agent deployed</span></span>                   | <span data-ttu-id="8ccd2-508">% windir%\\System32\\ccm\\-\\Protokolle\\Smstslog "smsts. log</span><span class="sxs-lookup"><span data-stu-id="8ccd2-508">%windir%\\System32\\ccm\\logs\\Smstslog\\smsts.log</span></span> |
| <span data-ttu-id="8ccd2-509">Ausführung der Task Sequenz abgeschlossen</span><span class="sxs-lookup"><span data-stu-id="8ccd2-509">Task sequence execution complete</span></span>                                                | <span data-ttu-id="8ccd2-510">% windir%\\System32\\ccm\\-\\Protokolle "smsts. log</span><span class="sxs-lookup"><span data-stu-id="8ccd2-510">%windir%\\System32\\ccm\\logs\\smsts.log</span></span>           |

> [!TIP]
> <span data-ttu-id="8ccd2-511">Sie können **F8** während der Tasksequenz zu einem beliebigen Zeitpunkt auswählen, um eine Befehlskonsole zu öffnen, und dann Zugriff auf die Datei "" smsts. log "erhalten.</span><span class="sxs-lookup"><span data-stu-id="8ccd2-511">You can select **F8** at any time during the task sequence to open a command console, and then get access to the SMSTS.log file.</span></span>

<span data-ttu-id="8ccd2-512">Wenn Sie Probleme mit dem PXE-Start beheben möchten, überprüfen Sie die beiden Protokolldateien auf dem Configuration Manager-Server, die für PXE-Aktionen spezifisch sind:</span><span class="sxs-lookup"><span data-stu-id="8ccd2-512">To troubleshoot PXE boot issues, check the two log files on the Configuration Manager server that are specific to PXE actions:</span></span>

-   <span data-ttu-id="8ccd2-513">**Pxecontrol. log**, befindet sich im Configuration Manager-Installationsprotokoll Verzeichnis</span><span class="sxs-lookup"><span data-stu-id="8ccd2-513">**Pxecontrol.log**, located in the Configuration Manager installation logs directory</span></span>

-   <span data-ttu-id="8ccd2-514">**"SMSPXE. log**, befindet sich im Configuration Manager-Verwaltungspunkt (MP)-Protokollverzeichnis</span><span class="sxs-lookup"><span data-stu-id="8ccd2-514">**Smspxe.log**, located in Configuration Manager Management Point (MP) logs directory</span></span>

<span data-ttu-id="8ccd2-515">Eine vollständige Liste der Protokolldateien, die Sie zur weiteren Problembehandlung bei der Installation von Configuration Manager verwenden können, finden Sie unter [Protokolldateien in System Center Configuration Manager](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files).</span><span class="sxs-lookup"><span data-stu-id="8ccd2-515">For a complete list of the log files that you can use to further troubleshoot your Configuration Manager installation, see [Log files in System Center Configuration Manager](https://docs.microsoft.com/sccm/core/plan-design/hierarchy/log-files).</span></span>
