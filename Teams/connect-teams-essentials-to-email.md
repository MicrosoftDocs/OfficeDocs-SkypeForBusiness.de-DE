---
title: Verbinden Microsoft Teams Essentials (AAD Identity) in ein vorhandenes E-Mail-System mit Kalender
author: adjoseph
ms.author: adjoseph
ms.reviewer: jimmyw
manager: serdars
...
ms.locfileid: "62279243"
---
# <a name="connect-microsoft-teams-essentials-aad-identity-to-an-existing-email-system-with-calendar"></a>Verbinden Microsoft Teams Essentials (AAD Identity) in ein vorhandenes E-Mail-System mit Kalender
...
2. Wechseln Sie im linken **Navigationsmenü** >  zu **E-Mail-& zusammenarbeitRichtlinienrichtlinien** >  & **RichtlinienBehandeln** >  von **RichtlinienAnti-spam** im Abschnitt Richtlinien
...
Um den Kalenderzugriff für Ihre lokalen Postfächer zu verwenden, folgen Sie den Anleitungen unterKonfigurieren des [Teams-Kalenderzugriffs für Exchange lokale Postfächer – Microsoft Tech Community](https://techcommunity.microsoft.com/t5/exchange-team-blog/configuring-teams-calendar-access-for-exchange-on-premises/ba-p/1484009)

Informationen zum Microsoft Teams-Räume in einer Hybridumgebung mit lokalem Exchange finden Sie unter Bereitstellen von Microsoft Teams-Räume mit Exchange [lokal – Microsoft Teams | Microsoft Docs](rooms/with-exchange-on-premises.md)
...
Alle in einer App Microsoft 365 E-Mails werden an Google Workspace weitergeleitet, sodass Benutzer Teams Erinnerungen und Benachrichtigungen erhalten. Benutzeridentitäten, wie die primäre E-Mail-Adresse des Benutzers, können dupliziert werden. Einmaliges Anmelden ist ebenfalls möglich, aber nicht erforderlich. Benutzer sollten an Besprechungen Teams kalender eines Drittanbieters oder über Teams teilnehmen können. Ein Teams Features funktioniert wie erwartet.
...
Diese Beispiele verwenden das [PowerShell-Verbinden-ExchangeOnline-PowerShell-Commandlet](/powershell/module/exchange/connect-exchangeonline?view=exchange-ps&preserve-view=true), das Teil [Exchange Online PowerShell V2-Moduls ist](/powershell/exchange/exchange-online-powershell-v2&preserve-view=true). Wenn Sie beim Ausführen von Verbinden-ExchangeOnline eine Fehlermeldung erhalten, stellen Sie sicher, dass Sie die empfohlenen Anweisungen zum Installieren des Moduls mit dem [EXO V2-Modul installiert haben](/powershell/exchange/exchange-online-powershell-v2?view=exchange-ps&preserve-view=true). Wenn Connect-ExchangeOnline zur Eingabe von Anmeldeinformationen aufgefordert werden, müssen Sie unbedingt ein Mandantenadministratorkonto verwenden.
...
7. Konfigurieren Sie DKIM (DomainKeys Identified Mail) für Microsoft 365, indem Sie die folgenden Schritte zum manuellen Einrichten [von DKIM ausführen](/microsoft-365/security/office-365-security/use-dkim-to-validate-outbound-email?view=o365-worldwide&preserve-view=true).

8. Melden Sie sich wieder beim Microsoft 365 Admin Center bei an<https://admin.microsoft.com/AdminPortal/>, um DKIM zu aktivieren

9. Wählen Sie im Navigationsbereich auf der linken Seite **SetupDomains**  >  aus.

10. Wählen Sie mithilfe des Kontrollkästchens Ihre vorhandene Nicht-Microsoft-Domäne (z. B. TomislavK@thephone-company.com) aus den aktuellen Domänenlisten aus.
...
13. **Bestätigen Sie set as default** in dem Fenster, das angezeigt wird, um Ihre vorhandene Domäne als Standard festlegen.
    ...
    Wenn Sie weitere Anleitungen zum Hinzufügen einer Domäne zu einer Microsoft 365 möchten, führen Sie die unter Hinzufügen [einer](https://support.office.com/article/add-a-domain-to-office-365-6383f56d-3d09-4dcb-9b41-b5f5a5efd611?ui=en-US&rs=en-US&ad=US) Domäne zu einer Domäne zu einem Microsoft 365.
...
2. Wechseln Sie zu **BenutzerAktive** >  Benutzer, und wählen **Sie Benutzer hinzufügen aus.**
...
Wenn Sie mehrere Benutzer gleichzeitig hinzufügen möchten, folgen Sie den empfohlenen Schritten unter Hinzufügen von Benutzern und Zuweisen von Lizenzen [– Microsoft 365 Administratorkonto | Microsoft Docs](/microsoft-365/admin/add-users/add-users?view=o365-worldwide&preserve-view=true)
...
2. Hinzufügen einer Route für Office 365
    ...
    **Smarthost-Methode zum Auflösen des ATTR35-Antwortcodes beim Senden von E-Mails an Exchange lokale/lokale Exchange Online:**
...
3. Konfigurieren der Route für Office 365
    ...
    - Wechseln Sie **zu** **AppsGoogle** >  **WorkspaceGmailRouting** >  > **.**
    ...
    - Damit **E-Mail-Nachrichten beeinflusst werden**, wählen Sie sowohl **"Eingehende"** als auch "  **Interner Empfang" aus.**

    - Wählen **Sie unter Für die oben genannten Nachrichtentypen** Nachricht **ändern aus.**
...
***Fügen Sie Ihre Unterdomäne in Google Workspace hinzu, um E-Mails von ihrem Microsoft 365.***
  ...
  Als Nächstes erstellen Sie Weiterleitungsregeln für Microsoft 365 an Ihre Unterdomäne. Wählen Sie eine Unterdomäne aus, die in Google Workspace zum Empfangen von E-Microsoft 365 verwendet werden soll (z. B. g.contosolandscaping2.m365master.com)

1. Starten bei **der Google Admin-Konsole** (admin.google.com)

2. Wechseln Sie **zu** **AccountDomainsManage** >  >  **Domains**
...
5. Wechseln Sie **zu** **AppsGoogle** >  **WorkspaceGmailCompliance** >  > .
...
15. Gehen Sie **im Feld Wenn die oben** angegebenen Ausdrücke übereinstimmen wie folgt vor, >  Nachricht ändern aus,  und aktivieren Sie unter Spam die Kontrollkästchen Spamfilter für diese Nachricht **umgehen**.
...
4. Wählen Sie **Exchange linken** Navigationsmenü des Admin Centers die Option **Mail** **flowConnectors** >  aus, öffnen Sie das Überlaufmenü (...) und wählen Sie Verbinder hinzufügen aus.
...
8. Wählen Sie **im Abschnitt Use of Connector** die Option **Only when I have a transport rule set up that redirects messages to this connector** aus, und wählen Sie **Weiter aus**.
...
*Weiterleiten von E-Microsoft 365 von Postfächern an Gmail*
...
1. Verwenden Sie **Microsoft 365 Admin Center,** um die einzelnen Postfächer zu aktualisieren, oder verwenden Sie ein **PowerShell-Skript** wie das folgende:
    ...
    **Problembehandlung Verbinden ExchangeOnline:**

    Tritt beim Ausführen von Verbinden ExchangeOnline ein Fehler auf? Dies kann das Ergebnis der Regel für die automatische E-Mail-Weiterleitung in Ihrer Organisation sein. Standardmäßig ist die automatische Weiterleitung deaktiviert. Damit Sie Teams Essentials mit Google Workspace verbinden können, muss die Regel aktiviert sein.  
...
*Konfigurieren Exchange Online transportregel "Direkt zum Kalender"*

1. Durch das Konfigurieren dieser Einstellung werden Kalender einladend automatisch akzeptiert, sodass sie in Teams Kalender angezeigt werden, ohne dass Benutzer mit der Einladung in der Kalenderansicht Outlook Web App.
...
1. Folgen Sie den Anweisungen unter [Aktivieren oder Deaktivieren Outlook im Web Postfachs in](/exchange/recipients-in-exchange-online/manage-user-mailboxes/enable-or-disable-outlook-web-app) Exchange Online, um die Outlook im Web für Postfächer zu deaktivieren.

2. Sie können Outlook im Web über Exchange **Admin Center oder** **PowerShell deaktivieren**. Sie können das folgende PowerShell-Beispiel verwenden, um die Outlook im Web für alle Postfächer zu deaktivieren:
...
4. Wählen **Exchange im Admin Center** im linken Navigationsmenü Nachrichtenfluss und dann Akzeptierte **Domänen aus.** 
...
**Schritt 6: Erstellen einer Regel zum Löschen aller eingehenden E-Mails Exchange Online mit Ausnahme von Kalender**
